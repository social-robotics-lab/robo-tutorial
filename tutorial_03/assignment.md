...
- 最新のGPT4を使ってよいが、文化情報学部の情報が言語モデルに十分に入っていない可能性がある。
  ...
  - 【LlamaIndexを使う時の注意】プログラムを実行するたびにインデックスを作成するようなプログラムを書くと、毎回大量のAPIコールが発生し、利用制限をすぐに超えてしまう。料金の無駄かつ他の人が使えなくなるので、絶対にやめること。インデックスを作成したら、ローカルに保存し、次回からはそれを読み込むようにせよ。よく分からなければ、飯尾研の先輩に相談すること。
- プロンプトの内容がキモになる。
...
## 提出方法
- e-lcassの第3回のレポート提出窓口に作成したプログラムをUPせよ（当日は各自のPCからロボットを操作してもらう）。
## 提出期限
- e-class上の指示を参照
